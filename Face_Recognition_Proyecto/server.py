...
try:
    # Usamos la fábrica pigpio para evitar temblores (requiere 'sudo pigpiod')
    factory = PiGPIOFactory()
    
    # Ajusta min_angle/max_angle si tu servo hace ruido al tope
    servo = AngularServo(17, min_angle=-90, max_angle=90, 
                         min_pulse_width=0.0006, max_pulse_width=0.0024,
                         pin_factory=factory)
    servo.angle = 0
    print("[INFO] Servomotor iniciado correctamente en GPIO 17.")
except Exception as e:
    print(f"[ADVERTENCIA] Error al iniciar Servo: {e}")
    print("¿Ejecutaste 'sudo pigpiod' en la terminal?")
    servo = None
...
# RUTA PARA CONTROL MANUAL DEL SERVO
